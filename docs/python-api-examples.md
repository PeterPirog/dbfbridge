...
| XLSX export / XLSX reconstruction | `pip install "dbfbridge[xlsx]"` |
...
The schema object is the authority used by `reconstruct_dbf`; the same
`to_dict()` payload is what the exporter writes as `<table>_schema.json`.
...
offset of the next page. `page.scanned` counts the physical records consumed
(deleted records included when `include_deleted=True`).
...
Base64 (key `__dbfbridge_raw_record__`-style payload handled inside
`to_dict()`), so always pass `record.to_dict()` — not raw Python bytes — to a
JSON transport.
...
result = export_dbf(
    ...
    "exported",        # output directory (created atomically)
    formats=("jsonl", "csv"),

...
Exact test evidence: closure-audit `main` suite = 432 passed, 1 skipped (433 collected, CI 33772195304); Macro A branch = 488 passed locally (Windows; includes the 55 new contract tests); `release/0.3.0` = exact-head release CI SUCCESS (454 passed, 2 skipped, release CI 33756842901).
...
- **Implementation (Macro A):** public `RawMode` Literal; `raw_mode` propagated through `ExportOptions`/`export_dbf`/`ExportConfig`/`make_config`/`run_export`/`_export_one`/CLI `--raw-mode`; incremental signature includes `raw_mode`; the shared physical loop runs with `keep_raw=False` for none/metadata (no raw allocation — §32); deleted=`separate` now feeds BOTH outputs from ONE shared-stream pass (raw images included in full-record; ordering/counts unchanged — §33); loss-aware `__dbfbridge_raw_text_fields__`/`__dbfbridge_binary_memo_fields__` retained in ALL modes (§30 decision: they are loss-aware logical/canonical aids, not physical blobs); `none` omits the replay-only `dbf.header_base64`/`memo.header_base64` schema blobs (reconstruction handles absence gracefully).
- **Explicit capability boundary (documented, tested):** nullable Varchar tables reconstruct canonically only in `full-record` mode — the schema-driven writer cannot yet rebuild the variable-length Varchar payload without the per-record raw image (documented writer limitation, compatibility-matrix row `V`); in `metadata`/`none` the reconstruction fails with the TYPED `DBF_RECORD_INVALID` detail (never an untyped crash).
- **Acceptance evidence:** `tests/test_raw_mode.py` — 33 tests (mode matrix, canonical reconstruction per mode incl. memo/nullable cases, raw-text fallback retained in every mode, deleted policies × modes, incremental invalidation, no-allocation spy, CSV column isolation, full-record regression). Benchmark evidence: `benchmarks/raw_mode_migration.py` (50k-record fixture: `none` 0.571 s / 4.89 MB output (1.21× source) vs `full-record` 0.614 s / 11.79 MB (2.91×) — output bytes −58.5%, wall −7%; measured locally, NOT a canonical baseline).
...
15. **(Macro A)** `RawMode` semantics (default `full-record`; loss-aware raw-text/binary-memo aids retained in every mode; `none` omits replay-only header blobs; nullable-Varchar full-record-only boundary).
...
- `python -m pytest -q`: **488 passed** (Windows local; 433 pre-Macro-A + 55 new contract tests). Exact CI counts recorded by the Macro A PR CI.

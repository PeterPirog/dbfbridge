...
CHANGELOG_UNRELEASED = CHANGELOG_FINAL.replace("## [9.9.9] - 2026-09-15", "## [9.9.9] - Unreleased")
README_FINAL = (
    "# dbfbridge\n\n"
    "> Status: **9.9.9 (alpha)**.\n\n"
    "This guide documents the dbfbridge 9.9.9 PyPI distribution.\n"
    "Check PyPI for currently available releases.\n"
)
README_PREP = README_FINAL.replace(
    "> Status: **9.9.9 (alpha)**.",
    "> Status: **9.9.9 (alpha)** — the 9.9.9 release is being prepared and has "
...
GUIDE_PENDING = GUIDE_FINAL.replace(
    "This guide documents the dbfbridge 9.9.9 PyPI distribution.",
    "The 9.9.9 publication is pending; the release is being prepared.",
)
...
def test_valid_final_release_state_passes(tmp_path) -> None:
    ...
    assert check_release_state.check_release_state("v9.9.9", root) == []
...
def test_invalid_tag_format_fails(tmp_path) -> None:
    root = _make_tree(tmp_path)
    violations = check_release_state.check_release_state("release-9.9.9", root)
    assert violations == [
        "tag 'release-9.9.9' does not encode a release version (expected v<pep440>, e.g. vX.Y.Z)"
    ]


def test_unreleased_changelog_section_fails(tmp_path) -> None:
    root = _make_tree(tmp_path, changelog=CHANGELOG_UNRELEASED)
    violations = check_release_state.check_release_state("v9.9.9", root)

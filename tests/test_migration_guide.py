"""Executable regression for the 0.2 -> 0.3 migration guide.

The representative snippet from ``docs/migration-0.3.md`` is not only
...
GUIDE = ROOT / "docs" / "migration-0.3.md"
...
def test_migration_guide_snippets_are_syntactically_valid() -> None:
    for block in _python_blocks():
        ast.parse(block, filename="docs/migration-0.3.md")

...
def parse_tag_version(tag: str) -> str | None:
    """Return the PEP 440 version encoded in *tag* (``vX.Y.Z`` -> ``X.Y.Z``)."""
...
def check_release_state(tag: str, root: Path | None = None) -> list[str]:
    ...
    if version is None:
        return [f"tag {tag!r} does not encode a release version (expected v<pep440>, e.g. vX.Y.Z)"]
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--tag",
        required=True,
        help="release tag being published, e.g. vX.Y.Z",
    )
    parser.add_argument(
        "--root",
        type=Path,

"""Shared physical DBF/FPT write backend — the ONLY DBF/FPT writer.

The implementation is derived from the proven reconstruction writer
(``importer/writer.py``, the correctness authority since Phase 0, including
the canonical Varchar logical-layout repair and the canonical ``_NullFlags``
contract of ``dbf_bridge.core.nullflags``).  The reconstruction pipeline and
the Direct Write API (``dbf_bridge.write``) use exactly one implementation:
one staging loop, one set of DBF field-type rules, one publication
transaction.

The backend consumes the reconstruction schema mapping (export-schema dict)
and record mappings with the ``__deleted__`` marker.  The typed direct-write
layer (:mod:`dbf_bridge.write`) adapts the public ``TableSchema`` and
``DirectRecord`` models into this contract and classifies failures at its
public boundary through the structured machine codes carried by
:class:`ReconstructionError` — never by parsing the English message.

The ``dbf`` backend is imported lazily when the writer actually moves bytes;
``import dbfbridge`` stays lazy and side-effect-free.  The canonical checksum
helpers stay in their single home (``dbf_bridge.importer.checksum``, the
reconstruction oracle tooling) and are imported on first use to keep the
...
if TYPE_CHECKING:  # pragma: no cover - typing only
    from ..importer.checksum import CanonicalChecksum
...
class ReconstructionError(ValueError):
    ...
    The single physical writer raises this error family; the reconstruction
    pipeline surfaces it verbatim, and the direct-write boundary maps it onto
    typed public errors.  Every raise site carries a structured machine
    ``code`` (a stable :class:`~dbf_bridge.core.errors.ErrorCode` member) so
    failures can be classified WITHOUT parsing the English message; the code
    is ``None`` for purely reconstruction-path failures that never cross the
    direct-write boundary.  An optional JSON-safe ``context`` mapping carries
    machine fields (e.g. the offending field NAME — never a record value) so
    the public boundary can build privacy-safe messages.
    """
...
def write_dbf(
    ...
    *,
    ...
    """Write one DBF (+FPT) table through staging with atomic publication.

    This is the shared physical writer: the reconstruction pipeline and the
    direct-write API both publish through it.  ``staging_directory`` lets a
    caller keep the ``.partial`` staging files outside the destination tree
    (defaults to the destination's own directory, the historical behaviour).
    """
    try:
        import dbf
    ...
    from ..importer.checksum import CanonicalChecksum, nullable_null_fields
    ...
    staging = Path(staging_directory) if staging_directory is not None else destination.parent
    staging.mkdir(parents=True, exist_ok=True)
    partial = staging / f".{destination.stem}.partial.dbf"
...
def _repair_varchar_logical_layout(
    ...
    with partial.open("r+b") as handle:
        ...
        from ..importer.checksum import nullable_null_fields
                ...
                if name in null_names:
                    ...
                    encoded = (
                        ...
                        if fallback is not None
                            ...
                            _encode_text(str(value), text_encodings, field=name)
...
def restore_raw_layout(
    ...
    every record can be matched safely.  This is the reconstruction oracle
    path: the direct-write API has no raw transport and never calls it.
    """
    ...
    try:
        if dbf_type in {"C", "V"}:
            return (
                ...
                else _encode_text(str(value), text_encodings, field=name)
            )
        ...
        # already machine-classified (e.g. WRITE_MEMO_FAILED for text
        # encoding) — preserve the code and context, never re-wrap it
        raise
...
def _patch_dbf_metadata(
    ...
    with path.open("r+b") as outfile:
        ...
        if len(header) != DBF_HEADER_SIZE:
            raise ReconstructionError(
                "Reconstructed DBF header is truncated.",
                code=ErrorCode.WRITE_SCHEMA_INVALID,
            )
        ...
        for index, field in enumerate(fields):
            ...
            if len(descriptor) != FIELD_DESCRIPTOR_SIZE:
                raise ReconstructionError(
                    f"Field descriptor {index + 1} is truncated.",
                    code=ErrorCode.WRITE_SCHEMA_INVALID,
                )
...
def _patch_fpt_metadata(path: Path, schema: Mapping[str, Any]) -> None:
    ...
    with path.open("r+b") as outfile:
        ...
        if len(generated) < 512:
            raise ReconstructionError(
                "Reconstructed FPT header is truncated.",
                code=ErrorCode.WRITE_SCHEMA_INVALID,
            )
...
def _raw_text_fields(record: Mapping[str, Any]) -> dict[str, bytes]:
    ...
    for name, encoded in value.items():
        try:
            ...
            raise ReconstructionError(
                f"Invalid raw text metadata for field {name!r}.",
                code=ErrorCode.WRITE_MEMO_FAILED,
            ) from exc
    return result
...
def _encode_text(value: str, encodings: list[str], field: str | None = None) -> bytes:
    errors: list[str] = []
...
def _update_numeric(value: Any, fielddef: Any, *_ignore: Any) -> bytes:
    ...
    try:
        ...
        raise ReconstructionError(
            f"Invalid numeric value {value!r}: {exc}",
            code=ErrorCode.WRITE_VALUE_INVALID,
            context={"field": str(fielddef[0])},
        ) from exc
    ...
    if len(rendered) > length:
        raise ReconstructionError(
            f"Numeric value {value!r} does not fit N/F({length},{decimals}).",
            code=ErrorCode.WRITE_VALUE_INVALID,
            context={"field": str(fielddef[0])},
        )
